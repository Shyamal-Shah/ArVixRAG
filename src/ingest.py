import json
from pathlib import Path
from urllib.request import urlretrieve

from arxiv import Client, Search, SortCriterion
from loguru import logger

try:
    from .models import Paper
    from .persistence import load_papers, save_papers, safe_filename
except ImportError:
    from models import Paper
    from persistence import load_papers, save_papers, safe_filename


class ArxivClient:
    def __init__(
        self,
        download_dir: str | None = None,
        metadata_path: str | None = None,
    ) -> None:
        self.client = Client()
        self.download_dir = (
            Path(download_dir) if download_dir else Path("data", "papers")
        )
        self.metadata_path = (
            Path(metadata_path) if metadata_path else Path("data", "metadata.json")
        )
        self.download_dir.mkdir(parents=True, exist_ok=True)

    def ingest_papers(self, query: str, max_results: int = 10) -> list[Paper]:
        new_metadata: list[Paper] = []
        try:
            logger.debug(f"Searching ArXiv with query: '{query}'")
            search_query = Search(
                query, max_results=max_results, sort_by=SortCriterion.SubmittedDate
            )
            papers = self.client.results(search_query)

            logger.info(f"Fetched papers from ArXiv with query: '{query}'")

            for i, paper in enumerate(papers):
                logger.debug(f"{i}. Downloading: {paper.title}")
                raw_path = self.download_dir.joinpath(
                    f"{safe_filename(paper.entry_id)}.pdf"
                ).as_posix()
                new_metadata.append(
                    Paper(
                        id=paper.entry_id,
                        title=paper.title,
                        authors=[a.name for a in paper.authors or []],
                        primary_category=paper.primary_category,
                        categories=paper.categories,
                        summary=paper.summary,
                        published=str(paper.published),
                        pdf_url=paper.pdf_url,
                        raw_path=raw_path,
                    )
                )
                if paper.pdf_url:
                    urlretrieve(paper.pdf_url, raw_path)
                logger.debug(f"{i}. Downloaded: {paper.title}")

            try:
                metadata = load_papers(self.metadata_path)
            except (FileNotFoundError, json.JSONDecodeError):
                logger.debug("Could not load metadata.json, starting fresh")
                metadata = []

            existing_ids = {p.id for p in metadata}
            deduped = [p for p in new_metadata if p.id not in existing_ids]
            if len(deduped) < len(new_metadata):
                logger.debug(
                    f"Skipped {len(new_metadata) - len(deduped)} duplicate papers"
                )
            metadata.extend(deduped)
            save_papers(metadata, self.metadata_path)

            return new_metadata

        except Exception as e:
            logger.error(f"Failed to ingest papers: {e}")
            raise


if __name__ == "__main__":
    arxiv_client = ArxivClient()

    results = arxiv_client.ingest_papers(
        query="all:transformer AND (cat:cs.AI OR cat:cs.LG OR cat:cs.CL OR cat:cs.CV)",
        max_results=50,
    )
    print(f"{len(results)} papers fetched")
