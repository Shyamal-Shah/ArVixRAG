    ...
    def ingest_papers(self, query: str, max_results: int = 10) -> list:
        ...
        try:
            ...
            for i, paper in enumerate(papers):
                ...
                local_path = papers_dir / f"{paper.title}.pdf"
                new_metadata.append(
                    {
                        "id": paper.entry_id,
                        "title": paper.title,
                ...
                if paper.pdf_url:
                    urlretrieve(paper.pdf_url, str(local_path))
                logger.debug(f"{i}. Downloaded: {paper.title}")

            metadata = []
            try:
                with open(Path("data", "metadata.json"), "r") as fp:
                    metadata = json.load(fp)
            except (FileNotFoundError, json.JSONDecodeError) as e:
                logger.debug(f"Could not load metadata.json, starting fresh: {e}")
            metadata.append(new_metadata)
            try:
                with open(Path("data", "metadata.json"), "w") as fp:
